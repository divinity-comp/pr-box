...
function updateProjectToServer() {
    ...
    }, {id:projectID,type:"project",valjs:JSON.stringify(projectdata)});
}
...
function toggleEventSettings(el) {
    ...
    if(el.hasAttribute("open")) {
        ...
        tl.set(evOpts, {x:-(elDimensions(idc("projectDate"),"All").x + 12),display:"block"})
        .staggerFromTo(evOpts.children, 0.2,{transformOrigin:"100% 0%",opacity:0,x:30,scale:0},{scale:1,opacity:1,x:0, ease: Circ.easeOut},0.05);
    }
}
...
function activateTimelineComp(el,eventType) {
    if(tData) {
        ...
        }, {projid:projectID,type:eventType});
    }
}
