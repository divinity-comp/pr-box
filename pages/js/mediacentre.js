...
function removeFile(ele) {
    ...
    var nameFind = eleFind.getElementsByClassName("dz-filename")[0].children[0].innerHTML;  findAndRemove(filesToAdd["doc"],"filename",nameFind);
...
function updateiframe(ele) {
    ...
    idc("videoAddov").children[0].children[0].src = videoLink;
}
...
function saveVideo() {
    ...
    var inputone = idc("videoAddov").children[0].children[1].value;
    var inputtwo = idc("videoAddov").children[0].children[2].value;
...
function uploadcoverageToServer() {
    ...
    var coverageJSONVString = JSON.stringify(coverageJSONV).replace("&", "/$");
     ...
     ajaxRequestToMake("data-calls/save-project-no-id.php",
        function(response) {
        console.log(response);
        if(response.indexOf('success') > -1) {
