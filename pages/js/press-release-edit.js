...
function moveElement(eNum) {
    ...
    if(selected) {
        if(eNum == -1) {
            if(selNum != 0) {prRelease.insertBefore(prRelease.children[selNum],prRelease.children[selNum - 1]);
            }
        }
        else {
...
function findImages() {
    navigator.camera.getPicture(uploadImageNow, failedUpload, { quality: 50,
...
function saveRelease() {
    ...
    if(pressReleaseForm[0].innerHTML != "") {
        var saveJson = {"releaseid":releaseid,"projectid":projectId,"revision":projrevision};
        ...
        for(i = 0; i < pressReleaseForm.length;i++) {
            ...
            var addDV = {"etype":pressReleaseForm[i].className,"name":prElName};
