...
function addTimelineRow(rowData) {
    ...
    tRow.ontouchstart = function (e) {
        ...
        if (deltaX < (elDimensions(activeI, "All").x / 2) + 30) {
            TweenMax.set(gbc("arrow", idc("activeInformation")), {
                x: -((elDimensions(activeI, "All").x / 2) - deltaX) - 30
            });
            deltaX = (elDimensions(activeI, "All").x / 2) + 30;
        } else if (deltaX > (elDimensions(gbc("overview"), "All").x) - (elDimensions(activeI, "All").x / 2)) {
            TweenMax.set(gbc("arrow", idc("activeInformation")), {
                x: -((elDimensions(gbc("overview"), "All").x) - (elDimensions(activeI, "All").x / 2) - deltaX)
            });
        ...
        TweenMax.set(activeI, {
            x: deltaX - (elDimensions(activeI, "All").x / 2) - findPos(gbc("overview")).x,
            y: deltaY
        });
    }
...
function editTimeline(project) {
    ...
    eActions.innerHTML = "<div class='hidden'>" + JSON.stringify(projJSON) + "</div>";

    idc("editTimeline").setAttribute("projId", projJSON["id"]);
    ...
    for (i = 0; i < projJSON["details"]["actions"].length; i++) {
        ...
        detailEl.innerHTML = '<div><button class="remove" onclick="removeEl(this.parentNode.parentNode);updateEventTimeline(this)">X</button><button onclick="toggleComplete(this);updateEventTimeline(this)" class="' + className + '"><i class="fa fa-check" aria-hidden="true"></i></button><span>' + actionDate.format('DD-MM-YYYY') + '</span></div><div contenteditable="true">' + projJSON["details"]["actions"][i]["name"] + '</div>';
    ...
    tl.staggerFromTo(editTimeline.getElementsByTagName("h2"), 0.35, {
        opacity: 0,
        y: 30
    }, {
        opacity: 1,
...
function updateEventTimeline(el) {
    var editTime = JSON.parse(idc("editTimeline").getElementsByClassName("eventActions")[0].children[0].innerHTML);
    editTime["details"]["name"] = idc("editTimeline").getElementsByTagName("h2")[0];
    editTime["details"]["actions"] = [];

    var eventsList = idc("editTimeline").getElementsByClassName("eventActions")[0].children;

    var adDate = eventsList[1].getElementsByTagName("span")[0].innerHTML.split("-");
    var dateAdj = new moment(editTime["details"]["date"]).diff(new moment(adDate[2] + "-" + adDate[1] + "-" + adDate[0]), 'days');
    console.log(dateAdj);
    for (i = 1; i < eventsList.length - 1; i++) {
        var eventSing = {
            name: "",
            days: "",
            active: ""
        }
        var aDate = eventsList[i].getElementsByTagName("span")[0].innerHTML.split("-");
        var date = new moment(aDate[2] + "-" + aDate[1] + "-" + aDate[0]).diff(new moment(editTime["details"]["date"]), 'days');

        eventSing.days = date + dateAdj;
        eventSing.name = eventsList[i].getElementsByTagName("div")[1].innerHTML;
        eventSing.active = eventsList[i].getElementsByTagName("button")[1].className;
        editTime["details"]["actions"].push(eventSing);
    }

    ajaxRequestToMake(urlInit + "/app/" + appVersion + "/update/project-event-details",
        function (response) {
            ...
            projid: idc("editTimeline").getAttribute("projid"),
            editData: JSON.stringify(editTime["details"])
        });
}

function timelineInit() {
    timeData.time = new moment();
    timeData.time.hour(0);
    timeData.time.minutes(0);
    timeData.time.seconds(0);
    timeData.time.milliseconds(0);
    xAxisDrag();
    timeselectionDrag();
    timelineType(0);
    gbc("viewport", idc("timeline")).style.height = "0px";
    var heightSet = 100;
    if (idc("main").className.includes("dash"))
        heightSet = elDimensions(idc("dashboard"), "All").y - elDimensions(idc("navigation"), "All").y - elDimensions(idc("timeline"), "Padding").y - 150;
    else
