...
function checkLogin() {
    ...
    function(response) {
        ...
        if(jsRes.response === "success") {
            ...
            checkLive = setInterval(function(){ checkIfLive(); }, 10000);
            getMainDashboard();
        }
        else {
            setTimeout(function(){ loginInit(); }, 2000);
        }
    }, {email:getCookie("email"),pass:getCookie("pass")});
    
}
function loginInit() {
    var loginPage = idc("loginPage").children[0];
    var tl = new TimelineMax();
    tl.fromTo(loginPage, 0.35,{opacity:0},{opacity:1, ease: Circ.easeOut})
    .fromTo(loginPage.children[0], 0.5,{y:30,transformOrigin:"50% 100%",scale:0,opacity:0},{y:0,scale:1,opacity:1, ease: Circ.easeOut})
    .fromTo(loginPage.children[1].children[0], 0.4,{y:30,opacity:0},{y:0,opacity:1, ease: Circ.easeOut},"-=0.25")
    .fromTo(loginPage.children[1].children[1], 0.4,{y:30,opacity:0},{y:0,opacity:1, ease: Circ.easeOut},"-=0.25")
    .fromTo(loginPage.children[1].children[2], 0.4,{y:30,opacity:0},{y:0,opacity:1, ease: Circ.easeOut},"-=0.25")
    .fromTo(loginPage.children[2], 0.4,{opacity:0},{opacity:1, ease: Circ.easeOut});
}
/* restUserDetails
delete_cookie("email");
delete_cookie("pass"); */
...
function loginOrPasswordReset(ev) {
    ...
    if(!buttonE.hasAttribute("clicked")) {
        ...
        if(passwordCheck.hasAttribute("prevPhrase")) {
            ...
            }, {email:formL[0].value,pass:formL[1].value});
        }
    }
}
function forgotPassword(passwordCheck) {
    ...
    if(passwordCheck.hasAttribute("prevPhrase")) {
        ...
        tl.to(passFieldHide, 0.35,{scale:1,marginBottom:passMB,height:login.children[0].offsetHeight});
        ...
        tl.fromTo(passFieldHide, 0.35,{scale:1,marginBottom:passMB + "px",height:passFieldDim.y + "px"},{marginBottom:"0px",height:"0px",scale:0, ease: Circ.easeIn,onComplete:function() {
...
function toggleAlerts() {
    ...
    if(alertSystem.getAttribute("open") == "true") {
        ...
        tl.set(notif, {
            x:xPos,y:yPos,height: (getHeight() - yPos) + "px",display:"block",maxWidth:xPos + "px"
        })
...
function successMessage(messageSuc) {
    ...
    setTimeout(function(){ 
        ...
        TweenMax.fromTo(sucMessage, 0.35, {
            x: "0%",opacity:1,width: sucWidth + "px",height:sucHeight + "px"
        }, {
            x: "-100%",opacity:0, width: "0px",
...
function toggleMobile(ele) {
    ...
    if (ele.getAttribute("active") == "true") {
        ...
        .staggerFromTo(pLinksLi, 0.2, {
            y: 30,opacity:0,scale:0,transformOrigin:"50% 100%"
        }, {
